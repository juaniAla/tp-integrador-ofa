...
public class RecetaService {
    ...
    public Receta guardar(Receta r) {
        if (!logica.autorPuedeCrearReceta(r)) {
            throw new RuntimeException("La receta no puede ser creada. El autor creo el maximo de recetas. Verifique los datos");
        }
        if (!logica.puedeAgregarIngredientes(r)) {
            throw new RuntimeException("La receta no puede ser creada. Excede la cantidad maxima de ingredientes");
        }
        if (!logica.costoIngredientesValido(r)) {
            throw new RuntimeException("La receta no puede ser creada. El monto de los ingredientes supera el maximo");
        }
